...
public class UserService {
    ...
    public void validateSetAdmin(User user, Long targetUserId) throws ValidationException {
        if (!user.isAdmin()) {
            throw new ValidationException("This action available only for admins");
        }
        if (user.getId() == targetUserId) {
            throw new ValidationException("You can't change your Admin permissions");
        }
    }

    public void setAdmin(Long targetUserId, boolean boolValue) {
        userRepository.setAdmin(targetUserId, boolValue);
    }
}
